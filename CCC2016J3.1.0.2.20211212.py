...
def get_max_palindrome_length(word, start, end, min_length) :
	assert start<end
	assert min_length>0
...
for index in range(word_length-1) :
	if index+longest_palindrome_length>=word_length :
		break
	palindrome_length = get_max_palindrome_length(word, index, word_length, longest_palindrome_length)
